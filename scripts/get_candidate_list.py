...
if __name__ == '__main__':
    ...
    all_candidates = {}
    all_candidates[BOS,BOS] = [BOS]
    all_candidates[EOS,EOS] = [EOS]
    for sent, pos_sent in zip(sent_list, pos_list):
        ...
        for i, (w, p) in enumerate(zip(words, pos)):
            ...
            if p.startswith('VB') and d.check(w) and w in vf:
                ...
                assert len(detxnf) > 0
                all_candidates[w,p] = all_candidates.get((w,p), []) +  detxnf
                ...
                assert len(detxjj) > 0
                all_candidates[w,p] = all_candidates.get((w,p), []) +  detxjj
            elif p.startswith('PRP'):
                #Pronouns
                #sys.stderr.write('in prp\n')
                all_candidates[w,p] = all_candidates.get((w,p), []) +  prof
            elif p.startswith('IN') and w in pf:
                all_candidates[w,p] = all_candidates.get((w,p), []) +  pf
            else:
                #sys.stderr.write(w + ' is not in any pos tag...\n')
                all_candidates[w,p] = all_candidates.get((w,p), []) +  [w]

    for (word,pos) , cans in all_candidates.items():
        can_set = set(cans + [word])
        print word  + '|||'+pos +  ' ### ' + '|||'.join(set(can_set))
